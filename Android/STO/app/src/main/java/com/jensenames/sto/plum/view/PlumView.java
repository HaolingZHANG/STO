...
public class PlumView extends SurfaceView {
    ...
    private void initView() {
        ...
        backgroundPaint.setColor(getResources().getColor(R.color.pathColor));
        backgroundPaint.setStyle(Paint.Style.STROKE);
        backgroundPaint.setAntiAlias(true);
        backgroundPaint.setDither(true);

        pathPaint = new Paint();
        pathPaint.setColor(getResources().getColor(R.color.colorPrimaryDark));
    ...
    private void drawMap(Canvas canvas) {
        ...
        pathPaint.setStrokeWidth(Math.min(getResources().getDimension(R.dimen.max_stroke_width), radius / 10));
    ...
    private void drawPath(Canvas canvas) {
        ...
        if(path.size() == 3) {
            ...
            if(isFirst)
                canvas.drawLine(points[min1][max1][0], points[min1][max1][1], points[min1][max1][2], points[min1][max1][3], pathPaint);

            canvas.drawLine(points[min2][max2][0], points[min2][max2][1], points[min2][max2][2], points[min2][max2][3], pathPaint);
        }
        path = null;
    }
}
